...
# from detector_head.yolov5.yolov_func import non_max_suppression


class Yolov5nPredictor:

    def __init__(self, model_path:str, input_size, fp16):
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.weights_path = model_path
        # 设置图像预处理参数，当然这里只是为了保证推理视频帧的和训练的数据增强保持一致
    ...
    def load_yolo_model(self):
        """weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, fp16=False, data=None, fuse=True"""
        model = DetectMultiBackend(self.weights_path, device=self.device, fp16=self.fp16)
        return model

    def predict(self, image, timer):
        timer.start()
        # 处理图像路径
        img_info = {"id": 0}
        ...
        im = torch.from_numpy(img).unsqueeze(0).float().to(self.device)  # 多张图的unsqueeze不一定是0 TODO
        if self.fp16:
            im = im.half()  # to FP16
        else:
...
if __name__ == '__main__':
    ...
    predictor = Yolov5nPredictor(model_path=model_path, input_size=(640, 640), fp16=False)
    img_path = r"D:\kend\myPython\Hk_Tracker\data\dataset\test_images\frame_0000.jpg"
    img = cv2.imread(img_path)
    output, _ = predictor.predict(img, MyTimer)
    print(output[0])
    # 将张量转换为NumPy数组
    # output_cpu = []
    # # 遍历元组中的每个张量
